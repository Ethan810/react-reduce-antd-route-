...
export default function createMode(createOpts) {
  const {
    mobile,
    initialReducer,
    defaultHistory,
  ...
  return function mode(hooks = {}) {
    ...
    function injectModel(createReducer, onError, unlisteners, m) {
      m = checkModel(m, mobile,this._store.dispatch);
      this._models.push(m);
    ...
    function unmodel(createReducer, reducers, _unlisteners, namespace) {
      ...
      store.replaceReducer(createReducer(store.asyncReducers));
      store.dispatch({ type: '@@mode/UPDATE' });
    ...
    function checkModel(m, mobile) {
      ...
      const model = { ...m };
      const { namespace, reducers, thunks } = model;
      ...
      function applyNamespace(type) {
        function getNamespacedReducers(reducers) {
          return Object.keys(reducers).reduce((memo, key) => {
            ...
            if(type ==='thunks') {
              model.reducers[`${namespace}${SEP}${key}`] = (state,{dispatch,payload})=>{
                dispatch(reducers[key](payload));
                return state;
              } ;
            } else {
    ...
    function prefixType(type, model) {
      ...
      if ((model.reducers && model.reducers[prefixedType])
        || (model.thunks && model.thunks[prefixedType])) {
        return prefixedType;
      }
      return type;
    }
